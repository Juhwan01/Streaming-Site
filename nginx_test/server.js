...
nms.on("postPublish", (id, StreamPath, args) => {
  ...
  if (streamKey) {
    ...
    console.error(
      "[postPublish] Invalid StreamKey for StreamPath:",
      StreamPath
    );
  }
});
...
nms.on("donePublish", async (id, StreamPath, args) => {
  ...
  if (streamKey) {
    ...
    // main.py 서버에 방송 종료 알림
    try {
      await axios.post("http://3.36.103.8:8001/stream_ended", {
        streamKey: streamKey,
      });
    } catch (error) {
    ...
    console.error(
      "[donePublish] Invalid StreamKey for StreamPath:",
      StreamPath
    );
  }
});
...
app.post("/stream-key", (req, res) => {
  ...
  res
    .status(201)
    .json({ message: "스트리밍 키가 성공적으로 추가되었습니다.", streamKey });
});
