...
const LiveStream = () => {
  ...
  const handleClick = (stream) => {
    ...
    router.push(
      `/broadcast?streamId=${stream.id}&title=${encodeURIComponent(
        stream.title
      )}&nickname=${encodeURIComponent(
        stream.nickname
      )}&profilePic=${encodeURIComponent(stream.profile_pic)}`
    );
  };
    ...
    <div className="w-full flex-col h-full">
        ...
        <div className="w-1/2 h-full bg-black shadow-2xl overflow-hidden relativ">
          <div className="w-full h-3/6 flex flex-col p-6 space-y-4 ">
            ...
            <div className="w-full flex items-center space-x-4 borer">
              ...
              <div className="flex items-center space-x-6">
                ...
                <div className="flex items-center bg-gray-800 px-3 py-1 rounded-full">
                  ...
                  <span className="text-lg pl-2 text-lime-500 font-medium">
                    Live 100명
                  </span>
                </div>
              </div>
            ...
            <div className="flex items-center space-x-2">
              ...
              />
              <span className="text-white text-xl font-medium pl-2">
                {currentSuggest.app}
              </span>
            ...
            <div className="flex space-x-4">
              {suggest.map((_, index) => (
                <GiPlainCircle
                  key={index}
                  className={`text-sm ${
                    currentIndex === index ? "text-lime-500" : "text-gray-600"
                  }`}
                />
              ))}
            </div>
      ...
      <div className="w-full h-1/2 flex flex-col">
        <div className="w-full h-1/6 flex items-center bg-black">
          <span className="text-2xl flex flex-row items-center justify-between px-6 text-white">
            <BsFire color="red" />
            실시간 인기 카테고리
          </span>
        </div>
      ...
      <div className="w-full h-1/2 flex flex-col">
        <div className="w-full h-1/6 flex items-center bg-black">
          <span className="text-2xl flex flex-row items-center justify-between px-6 text-white">
            <CiStar color="yellow" className="text-4xl" />
            현재 인기있는 스트리밍
          </span>
        </div>
