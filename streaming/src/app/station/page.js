...
const StatButton = ({ icon: Icon, count, className }) => (
  <button
    className={`h-12 flex items-center justify-center bg-white rounded-lg px-4 hover:bg-gray-100 transition-colors ${className}`}
  >
  ...
  useEffect(() => {
    const fetchUserInfo = async () => {
      try {
        ...
        const response = await axios.get("http://3.36.103.8:8001/users/me", {
          headers: {
            ...
            <div className="p-6 flex justify-between items-center">
              <div>
                ...
                <p className="text-gray-300">
                  광고/스폰 문의: {userInfo?.email || "이메일 정보 없음"}
                </p>
              </div>
          ...
          <div className="bg-gray-800 rounded-2xl p-6 mb-6">
            <div className="w-full flex flex-col items-center justify-center gap-3 mb-6 py-6 text-white">
              <p className="text-4xl">
                {userInfo?.full_name || "사용자"}님의 채널에 오신 것을
                환영합니다!
              </p>
